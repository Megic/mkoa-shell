...
module.exports = function ($this) {
    ...
    action['addNode']= async function () {//增加角色权限
        "use strict";
        let node= await $D('node').findById($this.POST['nodeId']);
        let role=await $D('role').findById($this.POST['id']);
        await role.addNode(node);//分配权限点
        $this.success();
    };

    action['removeNode']= async function () {//删除角色权限
        "use strict";
        let node= await $D('node').findById($this.POST['nodeId']);
        let role=await $D('role').findById($this.POST['id']);
        await role.removeNode(node);//删除权限节点
        $this.success();
    };

    action['getNode']= async function () {//获取角色权限
        "use strict";
        let role= await $D('role').findById($this.POST['id']);
        let nodes=await user.getNode();//所有拥有的权限
        $this.success(nodes);
    };

    action['add']= async function () {//新增
        "use strict";
        let node = await $D('role').create($this.POST);
        $this.success(node);
    };
    ...
    action['update']= async function () {//编辑更新
        ...
        await $D('role').update($this.POST, {where: where});
        $this.success();
    };

    action['findOne']= async function () {//查找
        "use strict";
        let node = await $D('role').findById($this.GET['id']);
        $this.success(node);
    };
    ...
    action['findAll']= async function (){//所有列表
        ...
        let where = {};//过滤
        let res = await $D('role').findAndCountAll({
            where: where,
            limit: perPages,
    ...
    action['delete']= async function () {//登录
        "use strict";
        await $D('role').destroy({where:{id:$this.POST['id']}});
        $this.success();
    };
